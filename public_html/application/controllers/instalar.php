...
class Instalar extends CI_Controller {
	...
	// Recebe as informacoes e faz a validacao para criar as configuracoes iniciais do sistema e as tabelas do banco de dados.
	public function index(){
		
		init_painel();
		$this->form_validation->set_rules('url_base', 'URL', 'trim|required|strtolower');
		$this->form_validation->set_rules('chave_seguranca', 'CHAVE DE SEGURAN&Ccedil;A', 'trim|required|strtolower');
		$this->form_validation->set_rules('tempo_sessao', 'TEMPO DA SESS&Atilde;O', 'trim|required|numeric');
		$this->form_validation->set_rules('hostname', 'SERVIDOR', 'trim|required');
		$this->form_validation->set_rules('username', 'USU&Aacute;RIO', 'trim|required');
		...
		if ($this->form_validation->run()==TRUE):
			...
			$sql_bd = "CREATE TABLE IF NOT EXISTS 'comentarios' (
			  'id_comentario' INT(11) NOT NULL AUTO_INCREMENT,
			  'id_artigo' INT(11) NOT NULL,
			  'nome_artigo' VRCHAR(255) NOT NULL,
			  'nome' varchar(100) NOT NULL,
			...
			$sql_bd = "CREATE TABLE IF NOT EXISTS 'mensagens' (
			  'id' INT(11) NOT NULL AUTO_INCREMENT,
			  'nome' varchar(100) NOT NULL,
		...
		set_tema('titulo', 'Instala&ccedil;&atilde;o do sistema');
		set_tema('conteudo', load_modulo('Instalar', 'instalar'));
		set_tema('rodape', '');
		load_template();
	}
	
	// Exibe a mensagem de sucesso da instalacao das configuracoes.
	public function sucesso(){
		init_painel();
		set_tema('titulo', 'Instala&ccedil;&atilde;o conclu&iacute;da');
